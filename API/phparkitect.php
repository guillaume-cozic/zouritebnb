...
return static function (Config $config): void {
    ...
    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Notification\Domain'))
        ->should(new NotDependsOnTheseNamespaces([
    ...
    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Notification\Application'))
        ->should(new NotDependsOnTheseNamespaces(['App\Notification\Infrastructure']))
        ->because('the application layer must not depend on infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Notification\Domain', 'App\Notification\Application'))
        ->should(new NotDependsOnTheseNamespaces([
            'Doctrine',
            'ApiPlatform',
            'Symfony\Bundle',
    ...
    // --- Donation module (hexagonal architecture) ---

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Donation\Domain'))
        ->should(new NotDependsOnTheseNamespaces([
            'App\Donation\Application',
            'App\Donation\Infrastructure',
        ]))
        ->because('the domain layer must not depend on application or infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Donation\Application'))
        ->should(new NotDependsOnTheseNamespaces(['App\Donation\Infrastructure']))
        ->because('the application layer must not depend on infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Donation\Domain', 'App\Donation\Application'))
        ->should(new NotDependsOnTheseNamespaces([
            'Doctrine',
            'ApiPlatform',
            'Stripe',
    ...
    // --- ActivityPoint module (hexagonal architecture) ---

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\ActivityPoint\Domain'))
        ->should(new NotDependsOnTheseNamespaces([
            'App\ActivityPoint\Application',
            'App\ActivityPoint\Infrastructure',
        ]))
        ->because('the domain layer must not depend on application or infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\ActivityPoint\Application'))
        ->should(new NotDependsOnTheseNamespaces(['App\ActivityPoint\Infrastructure']))
        ->because('the application layer must not depend on infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\ActivityPoint\Domain', 'App\ActivityPoint\Application'))
        ->should(new NotDependsOnTheseNamespaces([
            'Doctrine',
            'ApiPlatform',
            'Symfony\Bundle',
    ...
    // --- Contact module (hexagonal architecture) ---

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Contact\Domain'))
        ->should(new NotDependsOnTheseNamespaces([
            'App\Contact\Application',
            'App\Contact\Infrastructure',
        ]))
        ->because('the domain layer must not depend on application or infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Contact\Application'))
        ->should(new NotDependsOnTheseNamespaces(['App\Contact\Infrastructure']))
        ->because('the application layer must not depend on infrastructure (hexagonal architecture)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Contact\Domain', 'App\Contact\Application'))
        ->should(new NotDependsOnTheseNamespaces([
            'Doctrine',
            'ApiPlatform',
            'Symfony\Bundle',
    ...
    $rules[] = Rule::allClasses()
        ...
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
    ...
    $rules[] = Rule::allClasses()
        ...
        ->should(new NotDependsOnTheseNamespaces(['App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
    ...
    $rules[] = Rule::allClasses()
        ...
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Reservation'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Team'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Conversation'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Geography'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Payment'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Review'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Notification', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Notification'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Donation', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Donation'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\ActivityPoint', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\ActivityPoint'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\Contact']))
        ->because('modules must not depend on each other (vertical slicing)');

    $rules[] = Rule::allClasses()
        ->that(new ResideInOneOfTheseNamespaces('App\Contact'))
        ->should(new NotDependsOnTheseNamespaces(['App\Accommodation', 'App\SolidarityProject', 'App\Reservation', 'App\Team', 'App\Conversation', 'App\Geography', 'App\Payment', 'App\Review', 'App\Notification', 'App\Donation', 'App\ActivityPoint']))
        ->because('modules must not depend on each other (vertical slicing)');
