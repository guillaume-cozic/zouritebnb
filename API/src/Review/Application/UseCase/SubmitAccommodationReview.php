...
final readonly class SubmitAccommodationReview
{
    ...
    public function handle(SubmitAccommodationReviewCommand $command): void
    {
        $stay = $this->completedStayChecker->findCompletedStay($command->authorUserId, $command->accommodationId);
        if (null === $stay) {
            throw ReviewNotAllowedException::becauseStayNotCompleted();
        }

        if ($this->repository->existsForAuthorAndStay($command->authorUserId, $stay->reservationId, ReviewType::Accommodation)) {
        ...
        $review = Review::onAccommodation(
            ...
            subjectAccommodationId: $command->accommodationId,
