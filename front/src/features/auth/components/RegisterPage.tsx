    ...
    <AuthLayout>
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 mt-2">
          {isBookingContext ? t('auth.bookingRegisterSubtitle') : t('auth.registerSubtitle')}
        </p>
      </div>

      {isBookingContext && <BookingAuthNotice mode="register" />}

      <form onSubmit={handleSubmit} className="bg-white rounded-3xl border border-gray-100 shadow-sm p-7 sm:p-8 space-y-5">
        <div>
          <label htmlFor="register-email" className="block text-sm font-medium text-gray-700 mb-2">{t('auth.email')}</label>
        ...
        <div>
          <label htmlFor="register-password" className="block text-sm font-medium text-gray-700 mb-2">{t('auth.password')}</label>
          <div className="relative">
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
